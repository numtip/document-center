# EA / M365 Architecture Recovery — Gap Matrix
...
**WORK PC source (claimed):** `G:\ProjectAI\RAE-M365-Platform`  

---

## Purpose

Map every claimed EA / M365 architecture artifact from the historical WORK PC source against what actually exists in the canonical GitHub repository. Each artifact is classified to determine whether recovery from the WORK PC is needed.
...
| 3 | Expanded status model | More granular status values | **MISSING** | MISSING | GitHub has 4 status values (current, obsolete, archived, draft). The WORK PC may have additional states (e.g., pending-review, approved, published). |
...
| EA Phase | Artifacts Claimed | PRESENT_EQUIVALENT | PARTIAL | MISSING | Recovery Priority |
|----------|------------------|-------------------|---------|---------|-------------------|
...
**Decision: RECOVERY_REQUIRED_BEFORE_EA_3F**

### Evidence

The canonical GitHub repository is **not ready** to support EA-3F planning without recovering the missing WORK PC artifacts:
...
2. **SharePoint foundation missing**: EA-3F involves provisioning and configuring the M365 platform. Without the SharePoint site design, library schemas, content types, site columns, and permission groups from EA-3A, EA-3F would need to design these from scratch — duplicating work that reportedly already exists on the WORK PC.

3. **Provisioning automation**: EA-3B's PnP scripts represent automation logic that EA-3F would otherwise need to rebuild. Without them, EA-3F planning cannot reference the provisioning approach.

4. **Metadata foundation insufficient**: GitHub's 13-field registry model (with 10 required fields) is significantly simpler than the claimed 26-field model. EA-3F's governance and compliance requirements likely depend on the extended field set.
...
1. **Recover WORK PC artifacts** using the runbook at `docs/runbooks/EA_WORK_PC_RECOVERY_RUNBOOK.md`
