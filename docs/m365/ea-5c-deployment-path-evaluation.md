# EA-5C โ€” Power Automate Deployment Path Evaluation

**Phase:** EA-5C โ€” Power Automate Deployment Path Evaluation  
...
## 1. Capability Audit โ€” Installed Tools

### 1.1 Tool Inventory

| Tool | Installed | Version | Available To Install | Notes |
|------|:---------:|---------|:-------------------:|-------|
| `pac` (Power Platform CLI) | โ No | โ€” | โ… winget | `winget install Microsoft.PowerAppsCLI` |
| `az` (Azure CLI) | โ No | โ€” | โ… winget | Not needed for primary path |
| Python | โ… Yes | 3.14.2 | โ€” | โ€” |
| Python `requests` | โ… Yes | 2.33.0 | โ€” | HTTP capability available |
| Python `msal` | โ No | โ€” | โ… pip | `pip install msal` |
| Python `azure-identity` | โ No | โ€” | โ… pip | Alternative to msal |
| Node.js | โ… Yes | v24.18.0 | โ€” | โ€” |
| npm | โ… Yes | 11.8.0 | โ€” | โ€” |
| `dotnet` runtime | โ No | โ€” | โ… winget | Only needed for `pac package deploy` |
| PowerApps PowerShell module | โ No | โ€” | โ… PSGallery | `Install-Module Microsoft.PowerApps.Administration.PowerShell` |
| PnP PowerShell (SharePoint) | โ No | โ€” | โ… PSGallery | Unrelated to flow deployment |
...
|------|--------|---------|
| Browser authenticated to Power Platform | โ… ACTIVE | `make.powerautomate.com` loads with "Maejo university (default)" |
| MSAL access tokens in localStorage | โ… PRESENT | Multiple scopes including `https://service.powerapps.com//user_impersonation` and `https://api.powerplatform.com/โ€ฆ` |
| Tenant ID | โ… CONFIRMED | `8ec74a39-ddf6-41e1-b0a2-ff0459ea8eb8` |
| User account ID | โ… CONFIRMED | `6693e9ff-447f-4998-ba67-72a8791aadf1` |
| Token expiry | โ… VALID | Expires ~1 hour from session start (refreshable) |
...
|--------|:-----------:|-------------|
| **pac auth create (device code)** | โ… HIGH | Opens browser โ’ user confirms already-authenticated session. Interactive but one-time. |
| **pac auth create (username/password)** | ๐ก MEDIUM | Direct credentials. Not recommended; ROPC flow deprecated in many tenants. |
| **Python msal device code flow** | โ… HIGH | `msal.PublicClientApplication.acquire_token_by_device_flow()` โ’ browser prompt. Requires `pip install msal`. |
| **Python msal interactive browser** | โ… HIGH | Opens browser tab automatically. Leverages existing authenticated session. |
| **Service Principal (client credentials)** | ๐”ด LOW NOW | Requires Azure App Registration + admin consent. No App Registration configured yet. |
| **Browser session token extraction** | โ ๏ธ FRAGILE | Tokens in localStorage are valid but not repeatable / expire hourly. Not suitable for pipelines. |
...
# Step 2: Authenticate (device code โ€” leverages existing browser session)
...
|------------|-------------|----------------|
| Environment Maker role | Creating/importing solutions and flows | โ… CONFIRMED (Solutions UI shows "+New solution" button) |
| SharePoint connection (pre-existing) | Flow connection reference binding | โ… CONFIRMED (connections listed in Power Automate) |
| Approvals connector | WF-02 approval routing | โ… CONFIRMED (connector listed) |
| Office 365 Outlook connector | WF-01/WF-04 notifications | โ… CONFIRMED (connector listed) |
| Power Automate per-user or per-flow license | Running flows in production | ๐ก VERIFY (Microsoft 365 default license may be sufficient for non-premium connectors) |
| Premium connector license | Not needed (all connectors are standard) | โ… N/A |
...
|-------|--------|---------|
| Dataverse enabled in Default environment | โ… YES | Solutions page loads with full content |
| Solutions tab accessible | โ… YES | Make.powerautomate.com โ’ Solutions โ’ content displayed |
| Existing solutions present | โ… YES | 15+ managed solutions from Microsoft (Project, Scheduling, URS) |
| "+New solution" button accessible | โ… YES | Environment Maker confirmed |
| "Import solution" button accessible | โ… YES | Solution ZIP import available |
| "Connect to Git" button present | โ… YES | GitHub/Azure DevOps source control integration available |
...
- Full round-trip: PA UI โ’ Git โ’ PA import (or vice versa)
...
### 4.1 Method A โ€” pac CLI + Solution Import (RECOMMENDED)
...
- All flow definitions already specified in `ea-5b-workflow-implementation-report.md` ยง4
...
When importing a solution containing flows, each flow's connector must be bound to a **Connection Reference** โ€” a reusable reference to a pre-created connection.

**Required Connection References for WF-01 through WF-05:**

| Connector | API Name | Logical Name (canonical) | Status |
|-----------|---------|--------------------------|--------|
| SharePoint Online | `shared_sharepointonline` | `raedc_sharedsharepoint` | ๐”ด Need to create in settings file |
| Approvals | `shared_approvals` | `raedc_sharedapprovals` | ๐”ด Need to create in settings file |
| Office 365 Outlook | `shared_office365` | `raedc_sharedoutlook` | ๐”ด Need to create in settings file |
...
**ConnectionId resolution:** Navigate to `make.powerautomate.com โ’ Connections`, click each connection, copy the GUID from the URL. This is a one-time, 5-minute manual step.

### 4.2 Method B โ€” Dataverse Web API (Python + requests + msal)
...
**Dependencies:**
- `pip install msal` (not installed, audit complete โ’ can install now)
...
**Feasibility:** โ… MEDIUM-HIGH (requires msal install + CRM URL verification)

### 4.3 Method C โ€” Portal Solution Import (Manual UI / Semi-Automated)

**Classification:** `MANUAL BUT RELIABLE`

**Process:**
1. Create flows in Power Automate inside a **Solution** (not "My Flows")
2. Solutions โ’ Export solution โ’ Download ZIP
3. Commit ZIP and/or unpacked JSON files to git
4. For redeployment: Solutions โ’ Import solution โ’ upload ZIP โ’ map connections
...
### 4.4 Method D โ€” Power Platform GitHub Actions / Azure DevOps (CI/CD Pipeline)

**Classification:** `FUTURE STATE โ€” NOT NEEDED FOR EA-5B`
...
### 4.5 Method E โ€” Browser UI (Fallback Only)

**Classification:** `MANUAL โ€” LAST RESORT`
...
pac auth create `
  ...
  --deviceCode
# Follow browser prompt โ€” confirm with already-authenticated account
pac org who
```

#### Step 3: Resolve Connection IDs (one-time, 5 minutes)

Navigate to `make.powerautomate.com โ’ Connections`:
1. SharePoint connection โ’ copy GUID from URL โ’ paste to `settings/maejo.settings.json`
2. Approvals connection โ’ same
3. Office 365 Outlook connection โ’ same
...
โ””โ”€โ”€ solution-src/
    ...
    โ”โ”€โ”€ solution.xml                 โ solution manifest
    โ”โ”€โ”€ customizations.xml           โ component list
    โ””โ”€โ”€ Workflows/
        โ”โ”€โ”€ WF-01-UploadRegistration-Administration.json
        โ”โ”€โ”€ WF-01-UploadRegistration-FinanceProcurement.json
        โ”โ”€โ”€ WF-01-UploadRegistration-PlanningPolicy.json
        โ”โ”€โ”€ WF-01-UploadRegistration-AcademicServices.json
        โ”โ”€โ”€ WF-01-UploadRegistration-Research.json
        โ”โ”€โ”€ WF-01-UploadRegistration-SOPManuals.json
        โ”โ”€โ”€ WF-02-ApprovalLifecycle.json
        โ”โ”€โ”€ WF-03-ArchiveControl.json
        โ”โ”€โ”€ WF-04-ExpiringReviewNotification.json
        โ””โ”€โ”€ WF-05-ExportFoundation.json
```
...
**Option A โ€” pac (if available):**
```powershell
pac flow enable --name "WF-01 - RAE Upload & Registration - Administration"
```

**Option B โ€” Power Automate UI:**
Solutions โ’ [RAE-DC-Flows] โ’ Turn on each flow

**Option C โ€” Python (after msal install):**
```python
...
|---------|----------|------------|
| `pac` CLI not installed | ๐ก RESOLVABLE | `winget install Microsoft.PowerAppsCLI` โ€” no admin required |
| Connection IDs not captured | ๐ก RESOLVABLE | 5-minute manual step in PA Connections page |
| Flow JSON definitions not yet in solution format | ๐ก RESOLVABLE | Convert EA-5B specs to Azure Logic Apps JSON schema |
| Dataverse CRM URL not confirmed | ๐ก RESOLVABLE | `pac org who` after auth gives the URL |
| Category Owner assignments unresolved | ๐”ด DEPENDENCY | WF-02/WF-04 will route to Site Admin (TEST) until resolved |
| MigrationBot service account not provisioned | ๐”ด DEPENDENCY | WF-01 bulk path blocked; upload-by-staff path unaffected |
...
docs/m365/flows/
โ”โ”€โ”€ solution-src/
โ”   โ”โ”€โ”€ solution.xml
โ”   โ”โ”€โ”€ customizations.xml
โ”   โ””โ”€โ”€ Workflows/
โ”       โ””โ”€โ”€ WF-{N}-*.json
โ”โ”€โ”€ settings/
โ”   โ””โ”€โ”€ maejo.settings.json.template   โ template only (no ConnectionIds)
โ””โ”€โ”€ dist/
    โ””โ”€โ”€ .gitkeep                       โ RAE-DC-Flows.zip not committed (built artifact)
```
...
|---------|--------|:-----------:|-------|
| Power Platform Build Tools (GitHub Actions) | Microsoft official | โ… HIGH | Best for CI/CD; requires App Registration |
| `pac solution` CLI workflow | Microsoft official | โ… HIGH | **Primary recommended path** |
| Power Platform ALM Accelerator | Microsoft (AppSource) | ๐ก MEDIUM | Complex setup; overkill for 5 flows |
| Community `powerplatform-deploy` npm package | Community | ๐”ด LOW | Not maintained; use official pac |
| Direct Dataverse Web API | Microsoft official | โ… HIGH | Secondary path; requires msal |
| Import package via portal ZIP | Microsoft official | โ… HIGH | Manual fallback; always available |
...
| File | Change |
|------|--------|
| `docs/m365/ea-5c-deployment-path-evaluation.md` | **NEW** โ€” This report |
...
**EA5C_AUTOMATED_DEPLOYMENT_PATH_FOUND** โ…

### 11.1 Primary Path: `pac solution import` (pac CLI)

| Criterion | Assessment |
|-----------|------------|
| Repeatable | โ… YES โ€” same command every deployment |
| Version-controlled | โ… YES โ€” flow JSON in git repo |
| Microsoft-supported | โ… YES โ€” official Power Platform CLI |
| No browser automation required | โ… YES โ€” fully terminal-based |
| Admin privileges required | โ… NO โ€” Environment Maker role sufficient |
| Cost | โ… FREE โ€” pac CLI is free |
...
1. Install `pac` CLI via winget (**now allowed** โ€” capability audit complete)
...
5. `pac solution import` โ’ all 5 flows created in environment
...
**EA-5D โ€” Solution-Based Flow Implementation (pac CLI)**
...
**EA-5D will implement all 5 workflows and run end-to-end tests using the pac solution import path. If validated, final decision = EA5D_WORKFLOWS_VALIDATED โ’ EA-6.**
